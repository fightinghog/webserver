...
double chd_time_sum_w;
double chd_time_sum_r;
clock_t chd_tr1=clock();


 while ( (ret = read(file_fd, buffer, BUFSIZE)) > 0 ) {
 
clock_t chd_taa=clock();
(void)write(fd,buffer,ret); 
clock_t chd_tbb=clock();
double chd_tw= ((double)(chd_tbb-chd_taa))/CLOCKS_PER_SEC*1000;
chd_time_sum_w+= chd_tw;

}
    
clock_t chd_tr2=clock();
chd_time_sum_r =((double)(chd_tr2-chd_tr1))/CLOCKS_PER_SEC*1000;
chd_time_sum_r-=chd_time_sum_w;

clock_t ccc2=clock();
double ccc3;
ccc3 =((double)(ccc2-ccc1))/CLOCKS_PER_SEC*1000;
double ccc4,ccc5;
ccc4=chd_time_sum_r/ccc3;
ccc5=chd_time_sum_w/ccc3;

printf("r:%.5lfms w:%.5lfms sum:%0.5lfms r/sum:%0.5lf w/sum=%0.5lf (r+w)/sum:%0.5lf \n",chd_time_sum_r,chd_time_sum_w,ccc3,ccc4,ccc5,ccc4+ccc5);
}
